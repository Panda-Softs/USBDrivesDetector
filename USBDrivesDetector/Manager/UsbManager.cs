...
namespace USBDrivesDetector.Manager
{
    ...
    public class UsbManager : IDisposable
    {
        ...
        const string WMI_DISK_SEARCHER = @"select DeviceID, PNPDeviceID, Model from Win32_DiskDrive where InterfaceType='USB'";
        const string WMI_PARTITION_SEARCHER = @"associators of {{Win32_DiskDrive.DeviceID='{0}'}} where AssocClass = Win32_DiskDriveToDiskPartition";
        const string WMI_LOGICAL_SEARCHER = @"associators of {{Win32_DiskPartition.DeviceID='{0}'}} where AssocClass = Win32_LogicalDiskToPartition";
        const string WMI_LOGICAL_DISK_SEARCHER = @"select FreeSpace, Size, VolumeName, VolumeSerialNumber from Win32_LogicalDisk where Name='{0}'";
        #region DriverWindow
        ...
        public static void enumarateAllDisk()
        {

            //DriveType.Network;

            var vs = new ManagementObjectSearcher("select * from Win32_LogicalDisk").Get(); // where DriveType=3"

            foreach (ManagementObject volume in vs)
            {
                foreach (PropertyData prop in volume.Properties)
                {
        ...
        public static string GetVolumeSerialNumber(string volName)
        {
            //DriveType.Network = 3 for logical drive (mapped)
            ManagementObject volume = new ManagementObjectSearcher(String.Format("select * from Win32_LogicalDisk where Name='{0}'", volName)).First();
            string serialNumber = null;
            if (volume != null) {               
                var property = volume.GetPropertyValue("VolumeSerialNumber");
                if (property != null) serialNumber = property.ToString();
            }
            return serialNumber;
        }
